...
X的全称是X Window System。安装名称，是一个窗口系统。其实并不全面。X除了是一个窗口系统，还是一个图形渲染系统。X不仅提供窗口管理功能，也提供了图形绘制功能\footnote{而正是因为X提供了图形渲染，而导致了X成为性能低下的代名词。!\_!}。




\todo{X 是一个窗口系统，那么X为什么采用了 C/S 结构呢？}[-4ex]
