...
不同的运行级别对sysvinit本身并没有特殊含义。只是认为人为这样的运行级别而已。
...
我在这里强调了一个次序。因为服务和服务之间是有明显的依赖关系的。举些例子吧：所有的服务都依赖　/etc/init.d/root。 这个服务确保正确的检查了根分区并以读写模式重新挂载根分区\footnote{系统启动的时候是以只读模式挂载根分区的}。所以 /etc/init.d/root 必须首先启动并且是正常启动才能执行后续操作；所有的系统服务都依赖正确的系统时间，所以 /etc/init.d/hwclock 也必须尽早完成启动，该服务读取CMOS时钟并设置好系统时间和时区信息；提供HTTP服务的服务apache必须于mysql之后启动。因为php页面需要和mysql数据库交互；等等等等。
...
sysvinit大量依靠脚本（/etc/init.d/* ）实现系统启动过程。脚本的一大特定就是慢。
