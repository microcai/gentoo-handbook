...
提示︰ \\
\leftskip=2em
		\hbox to 1em {h} 每次按下就會向左移動。\\
		\hbox to 1em {l} 每次按下就會向右移動。 \\
		\hbox to 1em {j} 每次按下就會向下移動。 \\
		\hbox to 1em {k} 每次按下就會向上移動。 \\
\vskip 3ex
}
...
%对于键盘党\footnote{对惯用键盘操作电脑，提倡抛弃鼠标的人的称呼。}来说，让双手离开键盘是一种罪恶。没有比让双手保持在键盘上更能发挥打字的速度了。
