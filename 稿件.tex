...
\begin{figure}[h]%
\centering%
\includegraphics[scale=0.35]{pics/#1}%
\caption{#2}%
\end{figure}%
}
...
为了更加强化大型主机的功能，以让主机的资源可以提供更多使用者来利用，1964年，MIT\footnote{麻省理工大学}、通用电器（General Electric）和贝尔实验室\footnote{根据Multics Myths网站（一个网站致力于破除有关Multics的谣言）的说法，贝尔实验室应该是在1965年的时候才参加的。
}通力合作，计划开发一个完美的操作系统。

Multics计划的目的是想要让大型主机可以\textbf{}为超过300个以上的终端服务和一整套完整的信息处理系统。
Multics支持模块化的硬件配置：CPU、内存，磁盘等硬件都可以在需要的时候在系统运行的过程中被动态增加或移除。
